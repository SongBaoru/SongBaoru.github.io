...
如允许同时炼乳的客户端的数量、查询缓存的大小等都是MySQL的系统变量。
...
1. const
   ...
   访问时间是常数级的的访问方法被称为const访问方法。

   const访问方法常见的搜索条件是，主键或唯一二级索引列与一个常数进行等值比较。

2. ref

   扫描区间为单点扫描区间的访问方法被称为ref访问方法。

   ref访问方法常见的搜索条件是，非唯一的二级索引列与常数进行等值比较，或唯一索引列与NULL值进行等值比较。

3. ref_or_null

   ref_or_null访问方法指的是，搜索条件是非唯一的二级索引列与常数或NULL值进行等值比较。

4. range

   range访问方法指的是，搜索条件是若干单点扫描区间或范围扫描区间，但是扫描区间为（-∞, +∞）的访问方法不能称为range访问方法。
...
6. all
   ...
   all访问方法是使用全表扫描执行查询的访问方法。

7. index_merge

   index_merge（索引合并）访问方法是，使用多个索引来完成一次查询的执行方法。
